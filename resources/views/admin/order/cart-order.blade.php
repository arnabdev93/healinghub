   ...
   <div class="content-header">
      <div class="d-flex align-items-center">
         <div class="me-auto healbreadcrumb">
            <h4 class="page-title">Cart Orders</h4>
            <div class="d-inline-block align-items-center">
               <nav>
                  <ol class="breadcrumb">
                     <li class="breadcrumb-item"><a href="{{route('home')}}"><i class="mdi mdi-home-outline"></i></a></li>
                     <li class="breadcrumb-item active" aria-current="page">Cart Orders</li>
                  </ol>
               </nav>
            </div>
         </div>
...
$(document).ready(function () {
    $(document).ajaxComplete(function (event, xhr, settings) {
        if (settings.url && settings.url.indexOf('cart-orders') !== -1) {
            try {
                var json = JSON.parse(xhr.responseText);
                if (json && json.delivered_total !== undefined) {
                    $('#delivered_total_label').text('₹' + parseFloat(json.delivered_total).toFixed(2));
                }
            } catch(e) {}
        }
    });
